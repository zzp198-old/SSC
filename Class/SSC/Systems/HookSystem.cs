...
public class HookSystem : ModSystem
{
    ...
    {
        Console.WriteLine("IsLoadingEnabled HookSystem");
        return SSCConfig.Instance != null;
    }

    public override void Load()
    {
        NetMessage.SendData += IL_NetMessage_SendData; // 加入世界时无视GameMode
        MessageBuffer.GetData += IL_MessageBuffer_GetData; // SSC初始化
        Main.DrawInterface += IL_Main_DrawInterface; // SSC界面下隐藏其他界面
        Main.DoUpdate_AutoSave += IL_Main_DoUpdate_AutoSave; // 缩短自动保存间隔
        Player.InternalSavePlayerFile += IL_Player_InternalSavePlayerFile; // 重定向保存位置
        Player.KillMeForGood += IL_Player_KillMeForGood; // 硬核死亡删除云存档
    }

    public override void Unload()
    {
        NetMessage.SendData -= IL_NetMessage_SendData;
        MessageBuffer.GetData -= IL_MessageBuffer_GetData;
        Main.DrawInterface -= IL_Main_DrawInterface;
        Main.DoUpdate_AutoSave -= IL_Main_DoUpdate_AutoSave;
        Player.InternalSavePlayerFile -= IL_Player_InternalSavePlayerFile;
        Player.KillMeForGood -= IL_Player_KillMeForGood;
    }
    ...
    private static void IL_NetMessage_SendData(ILContext il)
    {
        ...
        ilCursor.EmitDelegate<Action<BinaryWriter>>(i => i.Write((byte)Terraria.Main.GameMode));
    }
    ...
    private static void IL_MessageBuffer_GetData(ILContext il)
    {
        ...
        ilCursor.GotoNext(MoveType.After,
            i => i.MatchLdarg(0),
            i => i.MatchLdfld(typeof(Terraria.MessageBuffer), nameof(Terraria.MessageBuffer.reader)),
        ...
        ilCursor.Emit(OpCodes.Ldfld, typeof(Terraria.MessageBuffer).GetField(nameof(Terraria.MessageBuffer.reader)));
        ilCursor.EmitDelegate<Action<BinaryReader>>(i =>
        {
            ...
            var data = new PlayerFileData(Path.Combine(Terraria.Main.PlayerPath, $"{QOS.ClientID}.plr"), false)
            {
            ...
            data.SetAsActive();
            Console.WriteLine("sscview");
            UISystem.UI.SetState(new SSCView()); // 唯一设置,界面只会在此时启用
        });
    }
    ...
    private static void IL_Player_InternalSavePlayerFile(ILContext il)
    {
        ...
        ilCursor.EmitDelegate<Action<TagCompound, byte[], PlayerFileData>>((obj, t, data) =>
        {
            ...
            mp.Write((byte)(data.Path.EndsWith($"{QOS.ClientID}.SSC") ? QOS.PID.CreateSSC : QOS.PID.SaveSSC));
