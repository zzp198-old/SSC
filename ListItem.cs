...
public class ListItem : UIPanel
{
    ...
    public ListItem(PlayerFileData data)
    {
        Data = data;
        ...
        {
            if (Main.LocalPlayer.GetModPlayer<MainPlayer>().SteamID == 0)
            {
                var p = ModContent.GetInstance<SSC>().GetPacket();
                p.Write((byte)SSC.ID.AskSelect);
            ...
            {
                SSCTools.Boot(Main.myPlayer, "Can't operate after connect");
            }
        };
        Append(PlayButton);
    ...
    {
        spriteBatch.Draw(_innerPanelTexture.Value, position, new Rectangle(0, 0, 8, _innerPanelTexture.Height()),
            Color.White);
    ...
    protected override void DrawSelf(SpriteBatch spriteBatch)
    {
        ...
        spriteBatch.Draw(Main.Assets.Request<Texture2D>("Images/UI/Divider").Value,
            new Vector2(x, innerDimensions.Y + 21f),
        ...
        var pos2 = position3 +
                   new Vector2((float)(width3 * 0.5 - FontAssets.MouseText.Value.MeasureString(text3).X * 0.5), 3f);
