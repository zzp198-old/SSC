...
public class NetCodeSystem : ModSystem
{
    ...
    public override void NetReceive(BinaryReader bin)
    {
        ...
        {
            ((Views.SSCView)UISystem.UI.CurrentState)?.FlashList(tag);
        }
    }
    ...
    public static void ChooseSSC(BinaryReader bin, int plr)
    {
        ...
        mp.Write((byte)Common.QOS.PID.LoadSSC);
        mp.Write(t.Length);
        mp.Write(t);
        mp.Write(tml.Length);
        mp.Write(tml);
    ...
    public static void LoadSSC(BinaryReader bin, int plr)
    {
        ...
        var data = new PlayerFileData(Path.Combine(Main.PlayerPath, $"{Common.QOSKit.ClientID}.SSC"), false) // 只有这里会设置后缀为SSC
        {
